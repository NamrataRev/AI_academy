...
- Explain the three major eras of AI — symbolic AI, machine learning, and deep learning
...
- Explain why the shift from symbolic AI to deep learning changed what AI could realistically do
...
The three eras:
- **Symbolic AI** — teach it every rule by hand
...
## Era 1 — Symbolic AI (1950s–1980s): Write Every Rule by Hand
...
This is called **symbolic AI** — knowledge is written by hand as logical rules, by human experts.
...
**Key insight:** ML systems improve as you give them more good-quality data — a very different mindset from symbolic AI's fixed rulebooks.
...
**Global example — Tesla Autopilot:**
...
timeline
    ...
    1950s : Symbolic AI — hand-written IF-THEN rules
...
| Term | Simple Meaning |
|---|---|
| **Symbolic AI** | AI built from hand-written IF-THEN rules by human experts |
...
**Global examples of LLMs in action:**
...
- **Perplexity AI** — searches the web and synthesises answers in real time, not just from training data
...
**Healthcare**
Early symbolic expert systems attempted rule-based diagnosis with limited success. Today's deep learning tools assist in reading X-rays with much higher accuracy. In India, LLM-based agents are beginning to help triage patient queries in regional languages.
...
```mermaid
flowchart TD
    A[Patient X-ray uploaded] --> B[Deep learning model\nscans for patterns]
    B --> C{Abnormality\ndetected?}
    C -- Yes --> D[Flag for doctor review\nwith confidence score]
    C -- No --> E[Mark as clear]
```
...
**Customer Support**
Older chatbots used rigid decision-tree scripts (symbolic-AI-style). Modern LLM chatbots hold natural conversations. Agent-based systems can actually check your order status and issue a refund — used globally by Amazon, Flipkart, and Zomato.
...
```mermaid
flowchart TD
    A[Customer: Where is my order?] --> B[Agent checks\norder database]
    B --> C[Agent checks\ndelivery status]
    C --> D[Agent replies with\nexact location and ETA]
```
...
## Worked Example — Classify These Four Systems

Look at these four systems and identify which era each belongs to:

| System | Era | Why |
|---|---|---|
| A 1970s medical program with hand-typed IF-THEN rules | Symbolic AI | Rules are written by human experts — no learning from data |
| A 1990s Gmail-style spam filter trained on labelled emails | Machine Learning | Learns statistical patterns from labelled data, no hand-written rules |
| A 2020s app that identifies crop disease from a leaf photo | Deep Learning | Uses a layered neural network trained on huge image datasets |
| A 2026 assistant that reads your calendar, checks train availability, and books your ticket | AI Agent | Uses tools, remembers context, plans multi-step actions |

**How to classify any AI system — ask three questions:**

1. Are the rules hand-written by a person, or learned from data? → If hand-written: Symbolic AI
...
- AI history moved through three broad eras: **Symbolic AI** (hand-written rules) → **Machine Learning** (learn patterns from data) → **Deep Learning** (learn very complex patterns using layered networks)
...
> **Interview tip:** Being able to explain *why* deep learning succeeded where symbolic AI failed — data availability, compute power, and neural network design — signals strong foundational understanding. Most freshers can't answer this. You now can.
