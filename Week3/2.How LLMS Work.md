...
Each token is converted into a long list of numbers called a **vector**. Think of it like giving every word a unique GPS coordinate in a giant mathematical map. Words that are similar in meaning end up close together on that map, and words that are unrelated end up far apart.

For example, the words "king" and "queen" would have vectors that are very close to each other — because they share similar meaning, context, and usage. But "king" and "bicycle" would be far apart.

```mermaid
flowchart LR
    A[Token: un, believ, able] --> B[Each token assigned\na vector - a list of numbers]
    B --> C[Numbers fed into\nthe neural network]
    C --> D[Model understands\nmeaning and context]
```

This is what allows the AI to understand **meaning**, not just match exact words. It's why when you ask about "a car," the AI also understands you might mean "vehicle" or "automobile" — because their vectors are mathematically close.
