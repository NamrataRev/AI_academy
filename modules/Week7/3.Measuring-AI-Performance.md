...
To make these ideas concrete, we'll follow one running example throughout: an online food delivery platform monitoring its own AI systems — starting with how consistently fast its order-confirmation bot responds, then digging into how well its fraud-detection AI catches fraudulent orders without blocking honest customers.
...
Back to our food delivery platform: suppose only 1 in 100 orders placed is actually fraudulent. If the platform builds a completely "dumb" AI that outputs "NOT FRAUD" for every single order without even looking at it, that AI will be right 99 times out of 100.
 
```
TP = 0   (never catches a fraudulent order)
TN = 99  (correctly waves through the 99 genuine orders)
FP = 0
FN = 1   (misses the 1 actual fraudulent order)
 
Accuracy = (0 + 99) / 100 = 99%
```
**99% accuracy — but the AI is completely useless.** It caught zero fraud. This is why accuracy alone is a dangerous metric whenever the dataset is unbalanced — which is almost always the case in real AI applications like fraud detection, disease screening, and defect detection.
...
**Example:** Our food delivery platform's fraud-detection AI flags 10 orders as fraudulent. Only 7 were actually fraudulent — 3 were legitimate orders wrongly blocked.
```
Precision = 7 / (7 + 3) = 7 / 10 = 0.70 = 70%
```
 
**When precision matters most:** When a false alarm (False Positive) is costly. For example:
- The platform auto-cancelling a genuine customer's order for suspected fraud — a real, hungry customer is left without dinner and probably won't order again
...
**Example:** There are 10 actually fraudulent orders hidden in that day's traffic. The AI only caught 6 — it let 4 fraudulent orders through.
```
...
- The platform's fraud detector — missing a real fraudulent order causes a direct financial loss it can never recover
...
**Example:** Using the fraud-detection numbers from above:
```
Precision = 0.70
Recall    = 0.60
...
### 🎯 Quick Check — Precision or Recall?

The same food delivery platform runs two very different AI systems:

**System A:** Flags orders as fraudulent and automatically holds them for manual review before the customer's card is charged.
**System B:** Screens new customer complaints for mentions of a food-safety risk (allergens, contamination) so a human can urgently investigate.

For each system, should the platform tune it to prioritise precision or recall? Think it through before revealing the answer.

<details>
<summary><b>Reveal answer</b></summary>

**System A leans toward precision.** Wrongly holding a genuine customer's order (a false positive) causes real frustration and lost trust, so the platform doesn't want to be trigger-happy about fraud.

**System B leans toward recall.** Missing a genuine safety complaint (a false negative) could mean a real health hazard goes uninvestigated — far worse than a human reviewer occasionally checking a false alarm.
...
Let's check back in on our food delivery platform's fraud-detection AI, now tested on two weeks of order data:
...
- Week 1 and Week 2 had different order volumes or customer demographics
...
- Following our food delivery platform end to end: median response time revealed the real user experience behind one bad outlier, a 99%-accurate fraud detector turned out to catch zero fraud, precision and recall exposed the real trade-off it was making, and a Week 1 vs Week 2 comparison caught it quietly becoming unreliable — one metric alone would have missed every one of these problems
