...
RAG (Retrieval-Augmented Generation) reduces hallucination by grounding answers in real retrieved documents — the model answers from actual current documents instead of only from memory. 

Think of it like the difference between a student answering from memory versus an open-book exam. Without RAG, the AI relies purely on what it learned during training — which may be outdated, incomplete, or simply misremembered. With RAG, before generating an answer, the system first searches a trusted knowledge base, database, or document library, retrieves the most relevant pieces of information, and then hands those to the AI as context. The AI then builds its answer from that retrieved material rather than guessing from memory.

This means RAG solves two problems at once — it reduces hallucination by giving the model real facts to work from, and it keeps answers current even after the model's training cutoff date, because the knowledge base can be updated independently of the model itself.

You will study RAG in full detail later in this program.
### Best Practices
