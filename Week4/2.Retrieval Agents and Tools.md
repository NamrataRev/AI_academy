...
```
- User asks a question → e.g., “What is the remote work policy?”
- System sends the question to the Vector Database to find relevant information.
- Vector Database performs a semantic search and finds the most relevant policy document/text.
- The retrieved information is sent back to the System.
- System adds the retrieved text to the prompt and tells the LLM to answer using only that information.
- LLM generates a factual answer based on the retrieved document.
- System delivers the answer to the User, ideally with citations or references to the source.
...
**A real business example — agent in action:**

**User request:** "Research the top 3 competitors, summarise their pricing, and save 
a PDF."

| Step | What the agent does |
|---|---|
| Thought | I need to find the top 3 competitors first. I will use the web search tool. |
| Action | Searches for "top market competitors" |
| Observation | Identifies Competitor A, B, and C from the results |
| Thought | Now I need their pricing. I will search each one separately. |
| Action | Searches "Competitor A pricing", then B, then C |
| Observation | Collects pricing data for all three |
| Final action | Uses a code tool to generate a PDF with all findings |
| Done | Hands the final PDF to the user |

Unlike a strict pipeline where step A always goes to step B, an agent is autonomous. 
If a search fails, it can decide on its own to rethink its plan and try a different 
search term — without you having to intervene.
...
That is exactly how tool use works:

1. The LLM decides it needs information or an action
2. It generates a request in a format the software can understand
3. The developer's software runs the actual code or API call
4. The result comes back to the LLM as text
