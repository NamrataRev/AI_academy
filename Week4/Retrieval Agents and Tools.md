...
- Standard foundation models are frozen in time, cannot verify facts, and cannot take actions in the real world.
- Retrieval-Augmented Generation (RAG) solves hallucinations by giving the AI access to external, factual documents at the moment a question is asked.
- Embeddings and Vector Databases convert text into mathematical representations to enable semantic search for RAG.
- An Agent combines an LLM with Memory, Tools, and a Planning Loop (like ReAct) to act autonomously towards a goal, instead of just chatting.
- Tool Use (Function Calling) allows an AI to trigger external software, like searching the web or querying an API, to execute real work.

> **Interview tip:** Be prepared to explain the ReAct (Reason and Act) loop. Walking an interviewer through the "Thought, Action, Observation" cycle demonstrates that you understand how agents plan and execute complex tasks.
