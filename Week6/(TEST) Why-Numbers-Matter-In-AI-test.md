...
- Always normalise numeric features before feeding them into an AI model to prevent features with naturally large numbers (like salary) from overwhelming features with small numbers (like years of experience).
- Explicitly document the encoding rules used for unstructured data, so anyone auditing the system understands how meaning was converted into numbers.
- Regularly review what real-world context is being left out during the encoding process — lost context is a primary source of bias.

## Common Beginner Mistakes

- Assuming AI models can directly "read" text or "see" images, rather than understanding that they only process the numeric representations of those inputs.
- Feeding raw, unnormalised data (like age and income) directly into a model and expecting the model to figure out the scale differences on its own.
- Believing that a single number can capture the full nuance of a complex real-world entity, rather than using a vector of multiple features.

---

## Real World Application

**AI Recommendation Systems — Netflix**
...
|---|---|
| Avg watch time (mins) | 85 |
| % watched to completion | 0.72 |
| Drama preference (1–10) | 8 |
| Comedy preference (1–10) | 4 |
| Thriller preference (1–10) | 7 |

This user is represented as: **[85, 0.72, 8, 4, 7]**.

Netflix mathematically compares this user's numbers to millions of others. Users with similar numbers tend to enjoy similar shows.

**What got left out?** Mood, language preferences, or if they're watching with family. The AI only knows what the numbers tell it.
...
```mermaid
flowchart TD
    A["Raw User Activity\n(clicks, views, ratings)"] --> B["Encoding & Normalisation"]
    B --> C["User Feature Vector\n[85, 0.72, 8, 4, 7]"]
    C --> D{"Compare to other\nusers' vectors"}
    D --> E["Find Nearest Neighbours\n(Similar users)"]
    E --> F["Generate Recommendations"]
```

**Three questions to ask for any AI system:**
1. What real-world information does the system need?
2. What numeric features capture that information?
3. What might be lost in that simplification?
