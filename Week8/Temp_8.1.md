...
## Similarity Scoring — Computing Cosine Similarity by Hand

### Before the Maths — What Are We Actually Measuring?

**Back to the job matching system:**

We have two candidate messages coming in through the system's chat interface:

- Candidate A writes: "I have experience building machine learning pipelines in Python"
- Candidate B writes: "I am skilled in Python for data science and model training"
- Candidate C writes: "I enjoy cooking Italian food at home"

**The question:** Which candidates are similar to each other, and which are unrelated? We need a number — a similarity score — to answer this mathematically.

The answer: we convert each message into a vector, and measure the **angle** between those two vectors. If the angle is small — they point in roughly the same direction — the messages are similar in meaning. If the angle is large — they are unrelated.
...
- **Negative score** — very rare in text embeddings, don't worry about this for now
...
**Why Step 4:** Without dividing, a longer vector always gets a bigger score even if it points in the same direction as a shorter one. Dividing removes this unfair size advantage.

---

### Worked Calculation — Matching Candidates in the Job System

**The setup:**
Three candidate messages arrive. We assign simplified 3D embeddings (three numbers per message):

| Message | Vector | What it is about |
|---|---|---|
| A: "I build ML pipelines in Python" | (0.9, 0.1, 0.2) | Technical, Python, ML |
| B: "Python for data science and model training" | (0.8, 0.2, 0.1) | Technical, Python, ML |
| C: "I enjoy cooking Italian food at home" | (0.1, 0.9, 0.8) | Cooking — nothing to do with tech |

**Our prediction before calculating:** A and B should be similar (both tech/Python). A and C should be unrelated (tech vs cooking). Let's verify with the maths.

---

**Calculation 1 — Candidate A vs Candidate B**

> The job system wants to know: are these two candidates a good match for the same role?

**Step 1 — Dot product** *(how much do these vectors overlap?)*
```
Position 1: 0.9 × 0.8 = 0.72
Position 2: 0.1 × 0.2 = 0.02
Position 3: 0.2 × 0.1 = 0.02
...
```
Large numbers line up in the same positions — strong overlap signal.

**Step 2 — Magnitude of A** *(how long is A's vector?)*
```
0.9² = 0.81,  0.1² = 0.01,  0.2² = 0.04
Sum = 0.86
Magnitude of A = √0.86 ≈ 0.927
```

**Step 3 — Magnitude of B** *(how long is B's vector?)*
```
0.8² = 0.64,  0.2² = 0.04,  0.1² = 0.01
Sum = 0.69
Magnitude of B = √0.69 ≈ 0.831
```
...
**Result: 0.99**
> Nearly identical meaning. The job matching system would recommend both candidates to the same Python/ML job posting. This matches our expectation.

---

**Calculation 2 — Candidate A vs Candidate C**

> Are a Python ML engineer and someone who likes cooking Italian food a match for the same role?
...
```
Numbers do not line up — low overlap signal.

**Step 2 — Magnitude of A:** Already calculated — **≈ 0.927**

**Step 3 — Magnitude of C**
```
0.1² = 0.01,  0.9² = 0.81,  0.8² = 0.64
Sum = 1.46
Magnitude of C = √1.46 ≈ 1.208
```
...
**Result: 0.30**
> Close to 0 — almost completely unrelated. The job matching system would never shortlist Candidate C for a Python ML engineering role. This matches our expectation.

---

**Summary:**

| Pair | Score | What the job system does |
|---|---|---|
| Candidate A vs B (both Python/ML) | 0.99 | Both shortlisted for the same role |
| Candidate A vs C (tech vs cooking) | 0.30 | C not shortlisted for a tech role |
...
- **Stopping at the dot product** — the dot product alone is not a similarity score. Always divide by both magnitudes
- **Mixing up positions** — always multiply position 1 with position 1. Never cross them
- **Worrying about negative scores** — almost all text similarity scores fall between 0 and 1 in practice
