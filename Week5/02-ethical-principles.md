...
3. Explain why all four pillars are needed together and why any one alone is not enough
4. Use the four pillars as a checklist when reviewing a proposed AI system design
5. Look at a simple system design and recommend improvements using the four pillars
...
**UPI payment fraud detection — a four-pillar review**

<div style="display:flex;align-items:flex-start;gap:24px;margin:16px 0">
<div style="flex:1">

A UPI fraud-detection AI that flags suspicious transactions must satisfy all four pillars:

- **Fairness** — tested across income groups, regions, and transaction types so it doesn't disproportionately block legitimate transactions from certain user groups
- **Transparency** — when a transaction is flagged, the user is told why — not just "transaction declined" with no explanation
- **Accountability** — a named team is responsible for the flagging logic, reviews false positives, and handles appeals
- **Harm prevention** — tested extensively before deployment to avoid wrongly freezing genuine transactions, with a fast human escalation path for disputed blocks
...
```mermaid
flowchart TD
    A[UPI transaction flagged\nas suspicious] --> B{Four-pillar check}
    B --> C[Fairness: tested across\nall user groups?]
    B --> D[Transparency: user told\nwhy it was flagged?]
    B --> E[Accountability: named team\nresponsible for review?]
    B --> F[Harm prevention: fast\nappeal and override path?]
    C & D & E & F --> G[Safe to deploy ✅]
```
