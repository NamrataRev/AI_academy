...
**Quality vs Quantity of Data — both matter, and both can cause bias:**

More data is not always better data. A model trained on a million examples of the same narrow type will still fail outside that narrow range. Think of it like studying for an exam using only one textbook — you might memorise it perfectly, but the moment the exam asks something from a different source, you're stuck.

- Quantity — the model needs enough examples of every group or situation it will encounter in the real world. Too few examples of a particular group means the model never learns accurate patterns for them. A fraud-detection model trained on mostly urban transactions will have too few rural transaction examples to make fair decisions for rural users.
- Quality — the examples must be accurate, representative, and free from pre-existing human bias. A million bad examples produce a confidently wrong model. Data quality problems include mislabelled records, outdated information, and examples that reflect historical unfairness rather than ground truth.

The real danger is when a model has high quantity but low quality — it appears to have learned well because it saw a lot of data, but what it actually learned is a biased or incorrect pattern, applied very confidently at scale.
