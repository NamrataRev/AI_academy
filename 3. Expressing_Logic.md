...
Remember the ATM pseudocode from Section 3.1? Here is the **exact same logic**, drawn as a flowchart. Trace it with your finger: start at the top, and every diamond asks you a question that decides which arrow to follow.
...
- Keep flowcharts readable — if you need more than 15–20 shapes, break the problem into smaller flowcharts (remember decomposition from Topic 3.3).
...
### 🚆 Railway Booking — IRCTC Waitlist

IRCTC's waitlist logic runs automatically for thousands of passengers the moment chart preparation begins.

- When chart is prepared, system checks if a berth was freed through cancellations
- If berth is available, ticket is confirmed and berth number assigned
- If not, passenger stays on waitlist — same logic, every time
...
```mermaid
flowchart TD
    Start([Chart prepared]) --> D{Berth available?}
    D -- Yes --> A[Confirm ticket\nAssign berth number]
    D -- No --> B[Remain on waitlist\nNo seat assigned]
    A --> End([End])
    B --> End
```
...
Return/refund policies are mapped as flowcharts so developers, support staff, and AI chatbots all follow the exact same logic.

- When a return is raised, system checks if the item is damaged
- If damaged, full refund is issued immediately
- If not, return window is checked to verify eligibility
...
```mermaid
flowchart TD
    Start([Return raised]) --> D{Item damaged?}
    D -- Yes --> A[Issue full refund\nProcessed immediately]
    D -- No --> B[Check return window\nVerify eligibility]
    A --> End([End])
    B --> End
```
